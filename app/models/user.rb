class User < ActiveRecord::Base
  ...
  attr_accessible :username, :email, :password, :password_confirmation, :remember_me, :uid, :provider, :name
  ...
  def self.find_for_google_oauth2(access_token, signed_in_resource=nil)
    ...
    unless user
        user = User.create(username: Devise.friendly_token[0,20],name: data["name"],provider: data['provider'], uid: data['uid'],
             email: data["email"],
...
def self.find_for_openid(access_token, signed_in_resource=nil)
  ...
  if user = User.find_by_email(data["email"])
    ...
    User.create!(username: Devise.friendly_token[0,20],name: data["name"],provider: data['provider'], uid: data['uid'],:email => data["email"], :password => Devise.friendly_token[0,20])
  end
end
